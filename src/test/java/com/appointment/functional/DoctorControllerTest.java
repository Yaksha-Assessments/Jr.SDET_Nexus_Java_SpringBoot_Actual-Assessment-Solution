...
import static com.appointment.utils.MasterData.getDoctorDTOList;
...
public class DoctorControllerTest {
	...
	public void testGetAllDoctors() throws Exception {
		List<DoctorDTO> doctorDTOs = getDoctorDTOList();

		when(this.doctorService.getAllDoctors()).thenReturn(doctorDTOs);
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/api/doctors")
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON);

		MvcResult result = mockMvc.perform(requestBuilder).andReturn();
		yakshaAssert(currentTest(),
				(result.getResponse().getContentAsString().contentEquals(MasterData.asJsonString(doctorDTOs)) ? "true"
						: "false"),
				businessTestFile);
	}
